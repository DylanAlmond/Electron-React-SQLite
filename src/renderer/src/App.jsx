...
const App = () => {
  ...
  return (
    <div className="flex gap-6 w-full h-full">
      <div className="flex flex-col gap-6 w-96 mt-0.5">
        <h1 className="block mb-2 text-2xl font-medium text-gray-900 dark:text-white">
          Selected: {currentTodo?.title || 'None'}
        </h1>
        <div>
          <label
            htmlFor="title"
            ...
            {todos &&
              [...todos]
                ...
                .map((todo) => (
                  ...
                  >
                    <th scope="row" className="px-6 py-4 max-w-40 overflow-hidden text-ellipsis">
                      {todo.id}
                    </th>
                    ...
                    <td className="px-6 py-4 max-w-40 overflow-hidden text-ellipsis">
                      {todo.dateCreated ? new Date(todo.dateCreated).toDateString() : '-'}
                    </td>
                    <td className="px-6 py-4 max-w-40 overflow-hidden text-ellipsis">
                      {todo.dateDue ? new Date(todo.dateDue).toDateString() : '-'}
                    </td>
